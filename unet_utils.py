...
class DoubleConv_down(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, mid_channels=256):
        ...
        self.double_conv_down = nn.Sequential(
            ...
            nn.BatchNorm2d(mid_channels),
            nn.ReLU(inplace=True),
            nn.Conv2d(mid_channels, mid_channels, kernel_size=2,stride = 2, bias=False),
            nn.BatchNorm2d(mid_channels),
            nn.ReLU(inplace=True),
            nn.Conv2d(mid_channels, out_channels, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(out_channels),
...
class DoubleConv_up(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, mid_channels=256):
        ...
        self.double_conv_up = nn.Sequential(
            nn.Conv2d(in_channels, mid_channels, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(mid_channels),
            nn.ReLU(inplace=True),
...
class Down(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.maxpool_conv = nn.Sequential(
            nn.MaxPool2d(2),
            DoubleConv_down(in_channels, out_channels)
        )

    def forward(self, x):
        return self.maxpool_conv(x)

class Downi(nn.Module):
    """Downscaling with maxpool then double conv"""
