...
class MultiscaleFlow(nn.Module):
    ...
    def forward_kld(self, x,yy, y=None):
        """
        Estimates forward KL divergence, see arXiv 1912.02762
        :param x: Batch sampled from target distribution
        :return: Estimate of forward KL divergence averaged over batch
        """
        return -torch.mean(self.log_prob(x, yy))

    def forward(self, x,yy,y=None):
        """
        ...
        :return: NLL
        """
        return -self.log_prob(x, yy)

    def sample(self,yy, num_samples=1, y=None, temperature=None):
        """
        Samples from flow-based approximate distribution
        :param num_samples: Number of samples to draw
        :param y: Classes to sample from, will be sampled uniformly if None
        ...
        for i in range(len(self.q0)):
            ...
            if i == 0:
                ...
                log_q += log_q_
                z, log_det = self.merges[i - 1]([z, z_],yy)
                log_q -= log_det
    ...
    def log_prob(self, x,yy, y=None):
        ...
        for i in range(len(self.q0) - 1, -1, -1):
            ...
            for j in range(len(self.flows[i]) - 1, -1, -1):
                z, log_det = self.flows[i][j].inverse(z,yy)
                log_q += log_det
            if i > 0:
                [z, z_], log_det = self.merges[i - 1].inverse(z,yy)
                log_q += log_det
            else:
                z_ = z
            if self.class_cond:
